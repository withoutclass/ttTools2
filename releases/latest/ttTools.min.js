...
ttTools.views.users.updateUser(vote[0]);});}},override_guestListName:function(){if(!Room.layouts.guestListName_ttTools)
...
setTimeout(function(){ttObjects.api({api:'room.vote',roomid:ttObjects.room.roomId,val:'down',vh:$.sha1(ttObjects.room.roomId+'down'+ttObjects.room.currentSong._id),th:$.sha1(Math.random()+""),ph:$.sha1(Math.random()+"")});console.log('Lamed')},250)},moveSongToBottom:function(fid){}}
...
div#playlistTools .custom-icons.dice { background-position:17px 0; }\
      "}).appendTo(document.head);$(util.buildTree(this.tree())).insertAfter($('.info .url').children().last())
$('.info .url').children().not('.ttTools2').remove()
$('div#buttons').buttonset();$('input#autoAwesome').click(function(e){if(ttTools.autoVote.enabled()!=='up')ttTools.autoVote.setEnabled('up');else ttTools.autoVote.setEnabled('false');ttTools.views.toolbar.update();ttTools.autoVote.execute();}).prop('checked',ttTools.autoVote.enabled()==='up').button('refresh');$('input#autoDJ').click(function(e){ttTools.autoDJ.setEnabled(!ttTools.autoDJ.enabled());ttTools.autoDJ.execute();}).prop('checked',ttTools.autoDJ.enabled()).button('refresh');$('button#casinoRoll').button({text:false,icons:{primary:'custom-icons dice'}}).click(function(e){$('div.chat-container form input').val('roll').parent().submit();});$('button#importExport').button({text:false,icons:{primary:'ui-icon-transferthick-e-w'}}).click(function(e){ttTools.portability.views.modal.render();});},tree:function(){return['div#playlistTools.ttTools2',{},['div#buttons',{},['input#autoAwesome',{type:'checkbox'}],['label',{'for':'autoAwesome'},['span.ui-icon.ui-icon-circle-arrow-n',{title:'Automatically upvote songs'}]],['input#autoDJ',{type:'checkbox'}],['label',{'for':'autoDJ'},['span.ui-icon.ui-icon-person',{title:'Attempt to get the next DJ spot'}]],['button#casinoRoll',{title:'Roll for a spot (casino mode)'}],['button#importExport',{title:'Import/Export'}]]];},update:function(){$('#autoDJ').prop('checked',ttTools.autoDJ.enabled()).button('refresh');$('#autoAwesome').prop('checked',ttTools.autoVote.enabled()==='up').button('refresh');}},users:{render:function(){console.log('appending')
$('<div/>',{id:'voteCount',class:'ttTools2'}).insertBefore($('#playlistTools'));$('<style/>',{type:'text/css',text:"\
...
div#guestDialog div.guest-list-container div.guests {\
  ...
  position:relative;\
      ...
      "}).appendTo(document.head);},renderOld:function(){$('<style/>',{type:'text/css',text:"\
div.guest.upvoter { background-color:#aea !important;}\
div.guest.upvoter:hover { background-color:#cec !important; }\
div.guest.downvoter { background-color:#eaa !important; }\
div.guest.downvoter:hover { background-color:#ecc !important; }\
...
div#guestDialog div.guest-list-container div.guests {\
  ...
  position:relative;\
      ...
      "}).appendTo(document.head);$('<div/>',{id:'guestDialog',class:'ttTools2'}).appendTo(document.body);this.setupDialog();},setupDialog:function(){},setupDialogOld:function(){$('div#guestDialog').dialog({width:285,height:350,title:' people are in this room',autoOpen:false,open:function(e,ui){$('div.chat-container div.guestListButton').hide();$('div.chat-container div.guestButton').show();$('div.guest-list-container').appendTo(this);$('div.guest-list-container div.chatBar').hide();$('div.guest-list-container div.chatHeader').hide();$('div#guestDialog').prev().find('span:first').prepend($('span#totalUsers'));},close:function(e,ui){$('div.chat-container div.guestListButton').show();$('div.chat-container div.guestButton').hide();$('div.guest-list-container').appendTo($('div#right-panel'));$('div.guest-list-container div.chatBar').show();$('div.guest-list-container div.chatHeader').show();$('span#totalUsers').prependTo('div.guestListSize');}});},update:function(){ttTools.views.users.updateVoteCount();$('div.guests .guest').each(function(index,element){ttTools.views.users.updateUser(element.id);});},modifyContainer:function(){$('div.guest-list-container div.guestListButton').after($('<div/>',{'class':'guestButton ui-state-default popout'}).append($('<div/>',{'class':'ui-icon ui-icon-newwin'})).bind('click',function(e){$('div#guestDialog').dialog('open');}));$('div.chat-container div.guestListButton').after($('<div/>',{'class':'guestButton ui-state-default',style:'border-left:0;'}).hide().append($('<div/>',{'class':'ui-icon ui-icon-pin-w'})).bind('click',function(e){$('div#guestDialog').dialog('close');}));},updateVoteCount:function(){console.log('Updating Voters')
...
guestName.append($(util.emojify(':notes:')).attr('title','Is on the decks'));},userStatus:function(uid){var averageActivity=ttTools.userActivityLog.averageActivity(uid);var threshold=ttTools.idleIndicator.threshold();if(averageActivity>threshold)return'red';if(averageActivity>(threshold/2))return'yellow';return'green';}},playlist:{render:function(){console.log('Do Nothing To Playlist')},renderOld:function(){$('<style/>',{type:'text/css',text:"\
...
.playlist-container .song.topSong .goBottom { display:none; }\
      "}).appendTo(document.head);},update:function(){},updateOld:function(){$('div.realPlaylist div.song div.goBottom').remove();$('<div class="goBottom"/>').on('click',function(e){e.stopPropagation();var song=$(this).closest('.song').data('songData');ttTools.moveSongToBottom(song.fileId);}).appendTo($('div.realPlaylist div.song'))}}}
...
ttTools.release=new Date(1345407716000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));